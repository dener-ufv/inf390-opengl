...
uniform sampler2D gSamplerShadowMap;
//0 NO TEXTURE, 1 REPLACE, 2 MODULATE
uniform int Texture_combiner;

float calcShadowFactor() {
    vec3 projCoords = vs_LightSpacePosition.xyz / vs_LightSpacePosition.w;
    vec2 uvCoords;
    uvCoords.x = 0.5 * projCoords.x + 0.5;
    uvCoords.y = 0.5 * projCoords.y + 0.5;
    float z = 0.5 * projCoords.z + 0.5;
    float depth = texture(gSamplerShadowMap, uvCoords).r;
...
void main()
{
    ...
    if (Texture_combiner == 0){
        FragColor = shadowFactor * vec4(rgb,1.0);
    }else if (Texture_combiner == 1){
        FragColor = shadowFactor * vec4(texel,1.0);
    }else{
        FragColor = shadowFactor * vec4( min(texel * scatteredLight + reflectedLight, vec3(1.0)), 1.0);
    }
}
